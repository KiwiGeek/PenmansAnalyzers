...
public class RadWindowShowAnalyzer : DiagnosticAnalyzer
{
    public const string DiagnosticId = "PARWP004";

    private static readonly DiagnosticDescriptor Rule = new(
        DiagnosticId,
        "Avoid calling Show on RadWindow",
        "Avoid calling the Show method on a RadWindow object",
        "Usage",
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext context)
    {
        ...
        if (context.SemanticModel.GetEnclosingSymbol(invocation.SpanStart) is INamedTypeSymbol { Name: "Wpf" } containingType &&
        ...
        // Check if the method is `Show` and the containing type is `RadWindow`
        if (methodSymbol.Name == "Show" &&
